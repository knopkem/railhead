...
- **A red baseline refuses to start** — `railhead feature` checks the project's verify is green first (run `railhead fix` before the feature), and a feature plan copies that suite rather than extending the global gate.
