...
- **Planning-time (richer, once).** `railhead plan` seeds the planner with a one-line summary of the index. Tickets are written to name exact files and existing symbols (contracts) they must honor, so the executor never has to "explore the codebase."
