 ...
 * by construction); `native` → OS-level input the railhead's ledger cannot
 * distinguish from any other bash command (exempt, gate row deferred);
 * `terminal`'s driven-stdin instance is not ledger-visible either (no
 * PTY/stdin event), so its gate row is deferred the same way; `none` → exempt
 * by construction (no surface to operate). A value whose instance is not
 * ledger-visible would carry a gate with no teeth — the prompt-only pollution
 * #97 argues against, one level down. Adding a value is a row of data here;
 * railhead.json values that match nothing throw rather than silently coerce, so
 * the addition is deliberate.
 */

export type ProjectInterface = "browser-ui" | "canvas" | "native" | "terminal" | "none";
 ...
 * `gameCanvasDefaults` prose, unchanged); `native` names the browser tools as
 * inapplicable and routes capture/input through the OS.
 */
export function buildInteractionGuidance(iface: ProjectInterface): string {
  switch (iface) {
    ...
    case "canvas":
      return `This is a canvas-based game or non-standard UI running in a browser page. The ${"`"}chrome-devtools_*${"`"} tools (click, fill, press_key) target DOM elements, but canvas apps handle input via ${"`"}window.addEventListener('keydown')${"`"}, ${"`"}canvas.requestPointerLock()${"`"}, or ${"`"}requestAnimationFrame${"'"}. There are no DOM buttons to click.\nTo interact with a canvas app, use ${"`"}evaluate_script${"'"}. To send keyboard input: dispatch ${"`"}KeyboardEvent('keydown', {key: 'w'})${"'"}. To simulate mouse-look: override ${"`"}document.pointerLockElement${"'"}. For pointer-lock games, also dispatch ${"`"}pointerlockchange${"'"}. For deterministic testing, stub ${"`"}Math.random${"'"}.`;
    case "native":
      return `This is a native desktop-window app — it opens its own OS window, with no browser page or DOM involved. The ${"`"}chrome-devtools_*${"`"} tools do not apply and cannot see it; do not open a browser to inspect it. Launch the built binary, capture the WINDOW with the platform's screenshot facility (a full-screen capture is acceptable when the window itself cannot be captured), and send input with the platform's keyboard/mouse automation or a test hook the plan named.`;
    case "terminal":
    case "none":
      return "";
  }
...
/** Whether the interface is a rendered user-facing surface — the set that gets
 * art direction, a coherence charter, and an implement-seat vision record;
 * `terminal`/`none` do not (a TUI's look is not planned as art, a library has
 * no surface). Every "rendered surface" decision goes through here so a new
 * surface value cannot land in one caller and miss another. */
export function isRenderedSurface(iface: ProjectInterface | null | undefined): boolean {
  return iface === "browser-ui" || iface === "canvas" || iface === "native";
}
 ...
 * nothing to operate). `native`'s OS-level input and `terminal`'s driven
 * stdin are not distinguishable from any bash command in the phase ledger (no
 * PTY/stdin or window event), so both gate rows are DEFERRED: a value whose
 * instance isn't ledger-visible would carry a gate with no teeth, one layer
 * down from the prompt-only pollution #97 argues against. Null/undeclared is
 * never enforced — that is today's behavior. */
