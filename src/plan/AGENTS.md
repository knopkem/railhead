...
- `planner.ts` — `runPlan`: one call for a fix; design → ticket decomposition for a build, with an optional post-plan interview, an interactive PLAN.md review, a plan gate, and the deterministic test-hardener ticket around it. Feature mode (ADR 0051) selects `planFeature*` prompts, injects the arc brief + held charter, and writes plan docs into the `.scratch/<slug>/` namespace. Also `runProductSession` (arc condense, never writes — adoption is the CLI's) and `deriveFeaturePrompt` (roadmap step → feature prompt), both one stage call through the shared `plannerStage`. Also `runSharpenSession` and `maybeGenerateAgentsMd` (writes a MINIMAL root AGENTS.md for the target project, only when none exists).
- `plan.ts` — prompt builders and the lossy block parsers: `planDesignSystemPrompt`, `planTicketsSystemPrompt`, `planFeatureDesignSystemPrompt`/`planFeatureTicketsSystemPrompt`, `planFixSystemPrompt`, `planContinuationSystemPrompt`, `buildProductSessionPrompt`/`parseProductReply`, `buildFeatureStepPrompt`/`parseFeatureStepReply`, `buildPlanGatePrompt`, `parsePlanGateVerdict`, `parsePlanJson`, `parseVerifyBlock`, `parseSmokeBlock`, `parseDesignBlock`, `parseArchitectureBlock`, `parseInterfaceBlock`, `splitCoherenceContract`.
...
- `plan-identity.ts` — `origin.json` (with `base_sha` for history-scoped ticket pre-marking); `checkPlanOrigin` refuses to resume a plan against a changed prompt.
- `plan-contradiction.ts` — detects a resolver failure that contradicts a plan-authored artifact name.

## Invariants

- Planning is exactly two model calls for a build (design, then tickets) and one for a fix. There is no model coverage audit, no plan-repair round, and no `$PLAN` block: the ticket decomposition is validated deterministically (non-empty `what`, criteria present unless `open_ended`) and a truncated decomposition gets exactly ONE "emit the remaining tickets" continuation call. The product-arc (`runProductSession`) and feature-derivation (`deriveFeaturePrompt`) sessions are likewise ONE call each.
- Feature plans (ADR 0051) plan INTO an existing product: no scaffold-first or entry-point-once rules — the first ticket is an integration slice leaving the EXISTING verify suite green; the feature spine replaces the whole-app launcher spine; the arc brief and held charter are DECIDED context, never re-decided.
