...
- `product.ts` — the product arc (`docs/product.md`, ADR 0051): `parseProductPlan`/`renderProductPlan` round-trip, `nextArcAction` (the strict frontier — a `built`-but-unverified step blocks the next until marked done or reopened; `ArcStepIdentity` is the step↔run link), `renderProductBrief`, `setStepStatus` (machine-readable marker lines only; prose survives byte-for-byte; boundaries are fence-aware).
