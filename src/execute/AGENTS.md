...
- `run.ts` — `startRun`, `runLoop`, `processTicket`, `assembleBranch`, `protectedPaths`. The orchestration hub; it imports from every other module group by design. `startRun` reads the plan's `origin.json` and carries a feature plan's `arc_step` on the run state (ADR 0051), so the goal reviewer and run-end finalizer know which roadmap step the run builds. The interaction smoke runs once per committed group boundary; a marker-less builder turn whose ticket was already verify-green counts as the checkpoint (marker stays primary).
- `executor.ts` — the ONLY phase runner: `executeOpendCode`, `executeFreshPhase`, streamed event parsing, persistent worker, stall/timeout kills, `describeExecFailure`. Fresh subprocess per judging phase keeps context O(ticket) (ADR 0001); the Builder is one durable session reused across checkpoints (ADR 0022/0047). Request-ceiling crossings are telemetry-only by default (`configureContextGuard`; `context_guard: "kill"` opts in, ADR 0014 amendment).
